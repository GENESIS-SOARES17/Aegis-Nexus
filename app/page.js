...
export default function Home() {
  ...
  const handleCancelDCA = async () => {
    ...
    try {
      ...
      alert("Strategy canceled and funds returned!");
      setTimeout(() => refetch(), 4000);
    } catch (err) {
      console.error(err);
    } finally {
    ...
    <main className="h-screen w-screen bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-blue-900/40 via-slate-950 to-slate-950 px-4 py-4 flex flex-col overflow-hidden text-slate-100">
      ...
      <header className="max-w-7xl w-full mx-auto flex flex-row justify-between items-center pb-4 mb-4 border-b border-slate-800/80 shrink-0 gap-4">
        <div className="flex items-center gap-4 shrink-0">
          <div>
            <p className="text-xl tracking-wider text-blue-400 uppercase font-extrabold leading-none mb-1">Aegis Nexus</p>
            <p className="text-xs text-slate-400 font-medium leading-none">LiteForge Environment</p>
          </div>
        </div>

        {/* INFINITE TICKER BAR */}
        <div className="flex-1 max-w-2xl lg:max-w-3xl xl:max-w-4xl mx-2 bg-slate-950/60 border border-slate-800/60 rounded-xl h-12 flex items-center overflow-hidden relative shadow-inner">
      ...
      <div className="max-w-7xl w-full mx-auto grid grid-cols-1 md:grid-cols-2 gap-4 flex-1 min-h-0 pb-2">
        <div className="flex flex-col gap-4 h-full min-h-0">
          ...
          <div className="bg-slate-900/40 backdrop-blur-md p-4 rounded-xl border border-slate-800/80 shadow-xl shrink-0">
            <div className="flex justify-between items-center mb-2">
              <h3 className="text-xs font-bold uppercase tracking-wider text-slate-200 flex items-center gap-2">
                <RefreshCw size={12} className="text-emerald-400" /> Your Active Position
              </h3>
              {isConnected && <button onClick={() => refetch()} className="text-[11px] text-blue-400 hover:underline">Refresh</button>}
            </div>
          ...
          <div className="bg-slate-900/40 backdrop-blur-md p-5 rounded-xl border border-slate-800/80 shadow-xl flex flex-col justify-center flex-1 min-h-0">
            <div className="mb-2">
              <h2 className="text-sm font-bold flex items-center gap-2 text-slate-100"><Play size={14} className="text-blue-400" /> New DCA Order</h2>
            ...
            {!isConnected ? (
              ...
              <form onSubmit={handleCreateDCA} className="space-y-2.5">
                <div>
                  ...
                  <input type="text" placeholder="0x..." className="w-full bg-slate-950/80 border border-slate-800 rounded-lg p-2 text-xs text-slate-100 focus:outline-none focus:border-blue-500 font-mono" value={tokenOut} onChange={(e) => setTokenOut(e.target.value)} />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-[10px] font-bold text-slate-200 uppercase tracking-wide mb-1">Amount Per Buy (zkLTC)</label>
                    <input type="number" step="any" placeholder="0.1" className="w-full bg-slate-950/80 border border-slate-800 rounded-lg p-2 text-xs" value={amountPerTx} onChange={(e) => setAmountPerTx(e.target.value)} />
                  </div>
                  <div>
                    <label className="block text-[10px] font-bold text-slate-200 uppercase tracking-wide mb-1">Interval</label>
                    <select className="w-full bg-slate-950/80 border border-slate-800 rounded-lg p-2 text-xs text-slate-200" value={daysInterval} onChange={(e) => setDaysInterval(e.target.value)}>
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input type="number" step="any" placeholder="e.g. 0.05" className="w-full bg-slate-950/80 border border-blue-900/50 rounded-lg p-2 text-xs font-semibold text-slate-100" value={targetPrice} onChange={(e) => setTargetPrice(e.target.value)} />
                  </div>
                  <div>
                    <label className="block text-[10px] font-bold text-slate-200 uppercase tracking-wide mb-1">Total Deposit (zkLTC)</label>
                    <input type="number" step="any" placeholder="1.0" className="w-full bg-slate-950/80 border border-slate-800 rounded-lg p-2 text-xs" value={totalDeposit} onChange={(e) => setTotalDeposit(e.target.value)} />
                  </div>
                </div>
                <button type="submit" disabled={loading} className="w-full bg-blue-600 hover:bg-blue-500 text-white font-bold p-2.5 rounded-lg text-xs tracking-wide uppercase mt-1">
        ...
        <div className="flex flex-col gap-4 h-full min-h-0">
          <div className="flex bg-slate-900/60 p-1 rounded-xl border border-slate-800/80 shrink-0">
            <button onClick={() => setActiveTab('tokens')} className={`flex-1 flex items-center justify-center gap-2 py-2 text-xs font-bold rounded-lg ${activeTab === 'tokens' ? 'bg-blue-600 text-white' : 'text-slate-400'}`}><List size={14} /> Network Tokens</button>
            <button onClick={() => setActiveTab('calculator')} className={`flex-1 flex items-center justify-center gap-2 py-2 text-xs font-bold rounded-lg ${activeTab === 'calculator' ? 'bg-blue-600 text-white' : 'text-slate-400'}`}><Calculator size={14} /> DCA Calculator & Compare</button>
          </div>

          {activeTab === 'tokens' && (
            <div className="bg-slate-900/40 backdrop-blur-md p-4 rounded-xl border border-slate-800/80 shadow-xl flex flex-col flex-1 min-h-0">
              <div className="shrink-0">
                <h3 className="text-xs font-bold uppercase tracking-wider text-slate-200">Tokens Found on Network (LiteForge)</h3>
                <p className="text-[10px] text-slate-400 mb-2">Click the copy icon to fetch the address and auto-fill the form.</p>
              </div>
                    ...
                    {TOKENS_LIST.map((token, idx) => (
                      <tr key={idx} className="hover:bg-slate-800/20">
                        ...
                        <td className="py-2 font-mono text-slate-300 flex items-center gap-1.5 justify-end">
                          <span className="truncate max-w-[120px] text-slate-300">{token.address}</span>
                          {!token.isNative && (
                            <button type="button" onClick={() => copyToClipboard(token.address, idx)} className="text-slate-400 hover:text-white p-0.5 bg-slate-950/40 rounded border border-slate-800/60">
                              {copiedIndex === idx ? <Check size={10} className="text-emerald-400" /> : <Copy size={10} />}
                            </button>
                          )}
                          {token.isNative && <span className="text-[8px] bg-slate-800 text-slate-400 font-bold px-1 py-0.2 rounded">NATIVE</span>}
                        </td>
                      </tr>
                    ))}
                  </tbody>
          ...
          {activeTab === 'calculator' && (
            <div className="bg-slate-900/40 backdrop-blur-md p-4 rounded-xl border border-slate-800/80 shadow-xl flex flex-col flex-1 min-h-0 overflow-y-auto space-y-4">
              <div className="bg-slate-950/50 p-3 rounded-lg border border-slate-800/60">
                <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-2 flex items-center gap-1.5"><ArrowRightLeft size={10} className="text-blue-400" /> Asset Pair Blueprint</h4>
                <div className="flex items-center justify-between bg-slate-900/40 p-2 rounded border border-slate-800/40 text-xs">
                  <div className="text-center flex-1">
                    ...
                    <p className="text-[9px] text-slate-500 font-mono mt-0.5">Live: ${prices.zkLTC}</p>
                  </div>
                  ...
                  <div className="text-center flex-1">
                    ...
                    <p className="text-[9px] text-slate-500 font-mono mt-0.5">{prices[targetTokenSymbol] ? `Live: $${prices[targetTokenSymbol]}` : 'Address Input'}</p>
                  </div>
                </div>
              </div>

              <div className="space-y-2">
                <h4 className="text-[10px] font-bold text-slate-400 uppercase tracking-wider flex items-center gap-1.5"><Calculator size={10} className="text-blue-400" /> Strategy Projection</h4>
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div className="bg-slate-950/40 border border-slate-800 p-2 rounded-lg">
                    <span className="text-slate-400 block tracking-wide text-[9px] uppercase font-semibold">Total Rounds</span>
                    <span className="text-base font-black text-slate-100">{simTotalBuys > 0 ? `${simTotalBuys} buys` : '---'}</span>
                  </div>
                  <div className="bg-slate-950/40 border border-slate-800 p-2 rounded-lg">
                    <span className="text-slate-400 block tracking-wide text-[9px] uppercase font-semibold">Lifecycle</span>
                    <span className="text-base font-black text-slate-100">{simTotalDays > 0 ? `~ ${simTotalDays} days` : '---'}</span>
                  </div>
                </div>
                <div className="bg-gradient-to-r from-emerald-950/20 to-blue-950/20 border border-emerald-900/40 p-3 rounded-lg">
                  <span className="text-emerald-400 tracking-wide text-[9px] uppercase font-bold block mb-1">Est. Accumulation</span>
                  <div className="flex items-baseline gap-1.5">
                    <span className="text-xl font-extrabold text-emerald-300">{simTokensAcquired > 0 ? simTokensAcquired.toFixed(4) : '0.0000'}</span>
              ...
              <div className="bg-slate-900/60 p-3 rounded-xl border border-slate-800/80 shadow-inner mt-auto">
                <h4 className="font-bold text-slate-300 text-[11px] flex items-center gap-1.5 mb-0.5"><ShieldCheck size={14} className="text-blue-400" /> Non-Custodial Escrow</h4>
                <p className="text-[10px] text-slate-400 leading-normal">Your funds stay protected inside the automated smart contract until parameters trigger or you revoke.</p>
              </div>
            </div>
          )}
        </div>
